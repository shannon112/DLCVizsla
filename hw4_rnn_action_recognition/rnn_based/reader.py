...
if __name__ == '__main__':
    root = "../../hw4_data"
    dict = getVideoList(os.path.join(root, "TrimmedVideos/label/gt_train.csv"))
    action_labels = (dict['Action_labels'])
    video_names = (dict['Video_name'])
    video_categorys = (dict['Video_category'])

    for i,(action_label, video_category, video_name) in enumerate(zip(action_labels,video_categorys,video_names)):
            print(i,action_label,video_category,video_name)

    example = readShortVideo(root+"/TrimmedVideos/video/train",video_categorys[0],video_names[0])
    print(example.shape)
