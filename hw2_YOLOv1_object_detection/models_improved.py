...
class VGG(nn.Module):
    ...
    def __init__(self, features, output_size=1274, image_size=448):
        ...
        #self.linear2 = nn.Linear(4096, 7*7*26)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1) #1, 512*7*7
        x = self.yolo(x) #fully-connected layer
        x = torch.sigmoid(x) #non-linear activation function
        x = x.view(-1,7,7,26)
        return x
...
def Yolov1_vgg16bn(pretrained=False, **kwargs):
    ...
    if pretrained:
        vgg_state_dict = model_zoo.load_url(model_urls['vgg16_bn']) #loading pretrain parameters
